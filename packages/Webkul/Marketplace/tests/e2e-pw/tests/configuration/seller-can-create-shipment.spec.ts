...
test('Seller can see the Orders details', async ({ adminPage }) => {   
    ...
    /**
     * Checked the Status is Enable or Disable
     */
    const seller_create_product =  await adminPage.locator('//input[@id="marketplace[settings][seller][can_create_invoice]"]').isChecked();
     ...
     */
    await adminPage.goto('http://192.168.15.80/Bagisto-v222/mp-v/public/seller/orders');
    await adminPage.waitForTimeout(2000);
    /**
    ...
    if (seller_create_product) {
        console.log('Ship Button is Visible');
        
        /**
         * Expect that the button is visible if the status is Enable.
         */
        await expect(adminPage.locator('//a[contains(.,"Ship")]')).toBeVisible();

        await adminPage.click('//a[contains(.,"Ship")]');
        await adminPage.fill('//input[@id="shipment[carrier_title]"]', 'fsdgf');
        await adminPage.fill('//input[@id="shipment[track_number]"]', 'f34534');
        await adminPage.locator('[id="shipment\\[source\\]"]').selectOption('1');
        /**
         * Create the Shipment from the Seller end
         */
