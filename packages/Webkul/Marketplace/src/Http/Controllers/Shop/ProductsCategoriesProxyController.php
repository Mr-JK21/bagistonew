...
class ProductsCategoriesProxyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($product) {
            ...
            if ($sellerProduct) {
                abort_if(! $this->mpProductRepository->isAvailableForSale($sellerProduct), 404);
            }
            ...
            if ($sellerProduct) {
                marketplace_visitor()->setSeller($sellerProduct->seller)->visit($product);
            } else {
                marketplace_visitor()->visit($product);
            }
