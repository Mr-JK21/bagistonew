...
class AssignProductController extends Controller
{
    ...
    {
        if (empty(core()->getConfigData('marketplace.settings.product.seller_can_assign'))) {
            return back();
        }
        ...
        if ($product) {
            return back()->with('error', trans('marketplace::app.seller.products.assign.already-selling'));
        }

        $baseProduct = $this->baseProductRepository->findOrFail($id);

        if (! $this->sellerRepository->getAllowedProducts()->has($baseProduct->type)) {
            return back()->with('error', trans('marketplace::app.seller.products.assign.product-not-allowed', [
    ...
    public function store(ProductFromRequest $request, int $id): RedirectResponse
    {
        ...
        return to_route('seller.products.index')
            ->with('success', trans('marketplace::app.seller.products.assign.create-success'));
    }
    ...
    public function update(ProductFromRequest $request, int $id)
    {
        ...
        return to_route('seller.products.index')
            ->with('success', trans('marketplace::app.seller.products.assign.update-success'));
    }
